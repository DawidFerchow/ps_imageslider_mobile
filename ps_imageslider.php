...
class Ps_ImageSlider extends Module implements WidgetInterface
{
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            ...
            foreach ($languages as $language) {
                ...
                $slide->image_mobile[$language['id_lang']] = 'sample-' . $i . '-mobile.jpg';
            }
            $slide->add();
        }
    }
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            if (!Tools::isSubmit('has_picture') && (!isset($_FILES['image_mobile_' . $id_lang_default]) || empty($_FILES['image_mobile_' . $id_lang_default]['tmp_name']))) {
                $errors[] = $this->getTranslator()->trans('The mobile image is not set.', [], 'Modules.Imageslider.Admin');
            }
            ...
            foreach ($languages as $language) {
                ...
                if (isset($_FILES['image_mobile_' . $language['id_lang']]) &&
                    ...
                    if ($error = ImageManager::validateUpload($_FILES['image_mobile_' . $language['id_lang']])) {
                        ...
                        return false;
                    } elseif (!ImageManager::resize($temp_name_mobile, __DIR__ . '/images/' . $salt_mobile . '_' . $_FILES['image_mobile_' . $language['id_lang']]['name'], null, null, $type)) {
                        $errors[] = $this->displayError($this->getTranslator()->trans('An error occurred during the image upload process.', [], 'Admin.Notifications.Error'));
                    }
                    if (file_exists($temp_name_mobile)) {
                        @unlink($temp_name_mobile);
                    }
                    $slide->image_mobile[$language['id_lang']] = $salt_mobile . '_' . $_FILES['image_mobile_' . $language['id_lang']]['name'];
                } elseif (Tools::getValue('image_mobile_' . $language['id_lang']) != '') {
                    $slide->image_mobile[$language['id_lang']] = Tools::getValue('image_mobile_' . $language['id_lang']);
                }
    ...
    public function getSlides($active = null)
    {
        ...
        foreach ($slides as &$slide) {
          if (!Context::getContext()->isMobile() && !Context::getContext()->isTablet()) {
            ...
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_ . 'ps_imageslider/images/' . $slide['image_mobile']);
          }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance((bool) _PS_USE_SQL_SLAVE_)->executeS('
            SELECT hssl.`image`, hssl.`id_lang`, hssl.`image_mobile`,
            FROM ' . _DB_PREFIX_ . 'homeslider hs
            LEFT JOIN ' . _DB_PREFIX_ . 'homeslider_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)
            LEFT JOIN ' . _DB_PREFIX_ . 'homeslider_slides_lang hssl ON (hss.id_homeslider_slides = hssl.id_homeslider_slides)
        ...
        $fields_form = [
            'form' => [
                ...
                'input' => [
                    ...
                    [
                        'type' => 'file_lang',
                        'label' => $this->getTranslator()->trans('Image for mobile', [], 'Modules.Imageslider.Admin'),
                        'name' => 'image_mobile',
                        'required' => true,
                        'lang' => true,
                        'desc' => $this->getTranslator()->trans('Maximum image size: %s.', [ini_get('upload_max_filesize')], 'Admin.Global'),
